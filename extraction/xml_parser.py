...
def clean_and_parse_xml_string(xml_string: str):
    """
    Limpia el string XML y lo parsea, manejando errores de codificación.
    """
    # Intentar limpiar caracteres inválidos antes de parsear
    # Esto es una medida de seguridad, algunos XML pueden venir con caracteres problemáticos
...
def extract_nested_invoice_xml(attached_document_xml_path: str) -> Optional[str]:
    """
    Extrae el XML de factura anidado dentro del CDATA de un AttachedDocument.
    """
    namespaces = {
    ...
    try:
        ...
        if description_node is not None and description_node.text:
            # El texto del nodo Description contiene el XML anidado dentro de CDATA
            # El parser de ElementTree ya maneja el CDATA automáticamente
...
def parse_invoice_xml(xml_content: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if supplier_party:
            ...
            # Puedes añadir más campos como dirección, email, teléfono del proveedor si los necesitas
        ...
        if customer_party:
            ...
            # Puedes añadir más campos como dirección, email, teléfono del cliente si los necesitas
            customer_email_node = customer_party.find('.//cac:Contact/cbc:ElectronicMail', namespaces)
            invoice_data['correo_cliente'] = customer_email_node.text if customer_email_node is not None else None # Esto podría ser útil para asociar usuario
        ...
        if legal_monetary_total:
            ...
            # Sumar TaxAmount de todos los TaxTotal para el total de impuestos, o buscar el total general si existe
        ...
        # --- Fecha de Vencimiento (PaymentMeans) ---
