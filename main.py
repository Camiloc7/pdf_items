...
from ingestion.zip_handler import extraer_pdfs_de_zip 
    ...
    while True:
            ...
            if not correos_encontrados:
                ...
                for correo in correos_encontrados:
                    ...
                    for adjunto_path_temp in correo["adjuntos_temp_paths"]:
                        rutas_pdfs_a_procesar_final = []
                        temp_extraction_dir = None 
                        try:
                            if adjunto_path_temp.lower().endswith('.zip'):
                                logger.info(f"  Extrayendo PDFs desde ZIP: {adjunto_path_temp}")
                                temp_extraction_dir = tempfile.mkdtemp() 
                                rutas_pdfs_extraidos = extraer_pdfs_de_zip(adjunto_path_temp)
                                rutas_pdfs_a_procesar_final.extend(rutas_pdfs_extraidos)
                            else:
                                rutas_pdfs_a_procesar_final.append(adjunto_path_temp) 
                            def process_pdf(pdf_path):
                                try:
                                    extracted_data = process_invoice(pdf_path)
                                    if extracted_data:
                                        save_invoice_to_db(extracted_data)
                                        logger.info(f"Archivo '{pdf_path}' procesado exitosamente.")
                                    else:
                                        logger.warning(f"No se pudieron extraer datos de '{pdf_path}'.")
                                except Exception as e:
                                    logger.error(f"Error al procesar '{pdf_path}': {e}", exc_info=True)

                            with ThreadPoolExecutor(max_workers=4) as executor:
                                executor.map(process_pdf, rutas_pdfs_a_procesar_final)
                        except Exception as e:
                            logger.error(f"Error durante la ingesta de adjunto '{adjunto_path_temp}': {e}", exc_info=True)
                        finally:
                            ...
                            if temp_extraction_dir and os.path.exists(temp_extraction_dir):
                                ...
                                logger.info(f"  Directorio temporal '{temp_extraction_dir}' limpiado.")
        except Exception as e:
        ...
        for filename in inbox_files:
            if filename.lower().endswith(".pdf"):
                pdf_path_to_process = os.path.join(settings.PDF_INPUT_DIR, filename)
                ...
                try:
                    extracted_data = process_invoice(pdf_path_to_process)
                    if extracted_data:
                        extracted_data.update(email_metadata_for_invoice)
                        invoice_id = save_invoice_to_db(extracted_data)
                        if invoice_id:
                            processed_count += 1
                            destination_path_processed = os.path.join(settings.PDF_PROCESSED_DIR, filename)
                            shutil.move(pdf_path_to_process, destination_path_processed)
                            ...
                            shutil.move(pdf_path_to_process, os.path.join(settings.PDF_ERROR_DIR, filename))
                    else:
        ...
        if processed_count > 0:
            logger.info(f"Completado el procesamiento de {processed_count} nuevos PDFs en este ciclo.")
        else:
            logger.info("No se procesaron nuevos PDFs del inbox en este ciclo.")
        logger.info(f"Esperando {settings.PROCESSING_INTERVAL_SECONDS} segundos antes del siguiente ciclo de procesamiento de PDFs...")
