...
# import json
# from config.settings import settings
# from database.models import init_db, SessionLocal, Factura, ItemFactura # Asegúrate de importar ItemFactura
# from database.crud import InvoiceCRUD, CorrectedFieldCRUD, ItemFacturaCRUD # Importa ItemFacturaCRUD
# from extraction.pdf_reader import PDFReader
# from extraction.ocr_engine import OCREngine
# from extraction.regex_parser import RegexParser
# from extraction.nlp_parser import NLPParser
...
#         }
#         # Convertir montos a float si es necesario
...
#                 except ValueError:
#                     logger.warning(f"No se pudo convertir '{item_data_db[key]}' a float para el campo '{key}' del ítem. Se guardará como string.")

#         new_item = item_crud.create_item(invoice_id, item_data_db)
#         db_session.commit()
#         logger.info(f"Ítem agregado exitosamente a la factura {invoice_id} con ID: {new_item.id}")
#         print("Ítem agregado.")
#     except Exception as e:
#         db_session.rollback()
#         logger.error(f"Error al agregar ítem a la factura {invoice_id}: {e}")
#         print(f"Error: {e}")
#     finally:
#         db_session.close()
...
#         # Convertir montos a float si es necesario
#         for key in ["cantidad", "precio_unitario", "total_linea"]:
#             if key in item_changes and isinstance(item_changes[key], str):
#                 try:
#                     item_changes[key] = float(item_changes[key].replace('.', '').replace(',', '.'))
#                 except ValueError:
#                     logger.warning(f"No se pudo convertir '{item_changes[key]}' a float para el campo '{key}' del ítem. Se guardará como string.")

#         updated_item = item_crud.update_item(item_id, item_changes)
#         db_session.commit()
#         if updated_item:
#             logger.info(f"Ítem {item_id} de la factura {invoice_id} actualizado exitosamente.")
#             print("Ítem actualizado.")
#         else:
#             logger.warning(f"No se encontró el ítem {item_id} para actualizar en la factura {invoice_id}.")
#             print("Error: Ítem no encontrado.")
#     except Exception as e:
#         db_session.rollback()
#         logger.error(f"Error al actualizar ítem {item_id} de la factura {invoice_id}: {e}")
#         print(f"Error: {e}")
#     finally:
#         db_session.close()
...
#         db_session.commit()
#         if deleted:
#             logger.info(f"Ítem {item_id} de la factura {invoice_id} eliminado exitosamente.")
#             print("Ítem eliminado.")
#         else:
#             logger.warning(f"No se encontró el ítem {item_id} para eliminar en la factura {invoice_id}.")
#             print("Error: Ítem no encontrado.")
#     except Exception as e:
#         db_session.rollback()
#         logger.error(f"Error al eliminar ítem {item_id} de la factura {invoice_id}: {e}")
#         print(f"Error: {e}")
#     finally:
#         db_session.close()
...
#     if len(sys.argv) > 1:
#         command = sys.argv[1]
#         init_db() # Inicializa la DB para todos los comandos CLI
...
#                 corrected_value = sys.argv[5]
#                 apply_correction(invoice_id, field_name, original_value, corrected_value)
#                 print("Corrección de cabecera procesada. Ver logs para detalles.")
#                 sys.exit(0)
#             else:
#                 print("Uso incorrecto para apply_correction: python main.py apply_correction <invoice_id> <field_name> <original_value> <corrected_value>")
#                 sys.exit(1)
...
#                     print("Comando add_item procesado.")
#                     sys.exit(0)
#                 except json.JSONDecodeError:
#                     print("Error: item_data_json no es un JSON válido.")
#                     sys.exit(1)
#             else:
#                 print("Uso incorrecto para add_item: python main.py add_item <invoice_id> <item_json>")
#                 sys.exit(1)
...
#                     print("Comando update_item procesado.")
#                     sys.exit(0)
#                 except json.JSONDecodeError:
#                     print("Error: item_changes_json no es un JSON válido.")
#                     sys.exit(1)
#             else:
#                 print("Uso incorrecto para update_item: python main.py update_item <invoice_id> <item_id> <item_changes_json>")
#                 sys.exit(1)
...
#                 print("Comando delete_item procesado.")
#                 sys.exit(0)
#             else:
#                 print("Uso incorrecto para delete_item: python main.py delete_item <invoice_id> <item_id>")
#                 sys.exit(1)
#         else:
...
import json
from config.settings import settings
from database.models import init_db, SessionLocal, Factura, ItemFactura
...
def main():
    ...
    if not os.path.exists(settings.PDF_INPUT_DIR):
        os.makedirs(settings.PDF_INPUT_DIR)
        print(f"Directorio de entrada de PDFs creado: {settings.PDF_INPUT_DIR}")
        print("Por favor, coloque archivos PDF de facturas en este directorio.")
        return
    ...
    try:
        ...
        print(f"Error al cargar/actualizar patrones de aprendizaje al inicio: {e}")
    ...
    if len(sys.argv) > 1:
        ...
        if command == 'apply_header_correction':
            if len(sys.argv) == 6:
                ...
                print("Corrección de cabecera procesada.")
                sys.exit(0)
            else:
                print("Uso incorrecto para apply_header_correction: python main.py apply_header_correction <invoice_id> <field_name> <original_value> <corrected_value>")
                sys.exit(1)
            ...
            if len(sys.argv) == 7:
                ...
                print("Corrección de ítem procesada.")
                sys.exit(0)
            else:
                print("Uso incorrecto para apply_item_correction: python main.py apply_item_correction <invoice_id> <item_id> <field_name> <original_value> <corrected_value>")
                sys.exit(1)
